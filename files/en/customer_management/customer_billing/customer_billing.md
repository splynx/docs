...
* **Blocking period (days after payment due)** - allows to extend the period when customer will be blocked (by default the value is `0`. The available values are `Do not block` and from `0` to `99` days after `Payment due`). The option `Do not block` allows to disable customer blocking, if the option is enabled, the customer deactivation won't work as well;
