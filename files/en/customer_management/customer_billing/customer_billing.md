...
* **Payment due (days after billing day)** - when the customer needs to make payments before being blocked. **Even though the customer will have a blocked status, their services will still be accounted** (default value is `15` days after `Billing day`);
* **Blocking period (days after payment due)** - allows to extend the period when customer will be blocked (by default the value is `0`). The option `Do not block` allows to disable customer blocking, if the option is enabled, the customer deactivation won't work as well;
