...
**NOTE:** Statistics of `Partially paid` invoices will be included in the `Unpaid` category of the **Totals** table.
...
**NOTE:** To **delete a payment associated with an invoice**, navigate to `Billing → Finance documents` in customer profile, choose `Payment` from the select menu of document type field and click on the <icon class="image-icon">![icon7](icon7.png)</icon> (Delete) icon next to the necessary payment item.
